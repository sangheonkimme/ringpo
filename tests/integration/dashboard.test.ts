...
describe("getDashboard", () => {
  ...
  it("aggregates per-automation stats, clicks, usage and waiting count", async () => {
    ...
    await createEvent(acct, { automationId: auto.id, status: "awaiting_follow" });
    ...
    expect(d.automations[0].stats).toEqual({ total: 6, succeeded: 2, partial: 0, failed: 1, pending: 2, skipped: 1, linksSent: 2, linksClicked: 1, clicks: 3 });
